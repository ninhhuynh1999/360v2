...
geometry = new THREE.PlaneGeometry(150, 150, 40, 40)
const plane = new THREE.Mesh(geometry.clone(), new THREE.MeshBasicMaterial({ wireframe: true }))
//scene.add(plane)
...
directionalLight.position.z = 81
...
const spotLightHelper = new THREE.SpotLightHelper(light, "red")
...
transformControls.attach(directionHelper);
...
let folder1 = gui.addFolder("SpotLight")
folder1.add(lineGUI, "spotlight")
folder1.add(light, "intensity", 0, 2, 0.001)
folder1.addColor(lightcolor, 'spotlight').onChange(function (val) {
...
folder3.addColor(lightcolor, 'directionalLight').onChange(function (val) {
    ...
    directionalLight.color = (val);
    render();

});

function updateGUI() {
    if (lineGUI.spotlight) {
        transformControls.attach(light)
    }
}
...
function gltf(url,item) {
    ...
    gltfLoader.load(url, (gltf) => {
       ...
       // console.log(gltf.scene)
        // if(gltf_Models[0].model == null){
        //     gltf_Models[0].model=gltf.scene
        // }
...
gltf("/models/gltf/a.gltf",gltf_Models[0])